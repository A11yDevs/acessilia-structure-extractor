...
def sanitize_text(text: str) -> str:
    """Remove caracteres de controle, leaks de prompt e metadados de audiodescrição."""
    if not text:
        return ""
...
def sanitize_block_text(text: str, block_type: str | None = None) -> str:
    """Sanitiza texto de um bloco, preservando código literal."""
    if not text:
        return ""
    if block_type == "code":
        return text.replace("\r\n", "\n").replace("\r", "\n")
