"""Processing manifest builder — constrói o ProcessingManifest a partir da extração Docling."""
...
from acessilia_extractor.pipeline.sanitizer import sanitize_text
from acessilia_extractor.pipeline.table_ast import normalize_table_ast
from acessilia_extractor.pipeline.table_ast import rows_from_table_ast

from acessilia_extractor.extractors import ExtractionResult
from acessilia_extractor.manifest.models import (
    BoundingBox,
    ExtractorRun,
    ManifestElement,
    ManifestSummary,
...
def build_processing_manifest(
    ...
    *,
    ...
    """Constrói um ProcessingManifest validado a partir de uma extração."""
...
def _build_elements(document: Any, *, enable_callouts: bool = True) -> list[ManifestElement]:
    """Constrói a lista de ManifestElement a partir do documento Docling."""
...
def _tag_known_title_callouts(page_elements: list[ManifestElement]) -> None:
    ...
    for index, title in enumerate(ordered):
        ...
        if len(group) >= 2:
            _apply_callout_group(title, group, callout_id=f"callout-known-{title.reading_order:06d}")
